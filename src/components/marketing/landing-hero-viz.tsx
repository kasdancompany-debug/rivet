 ...
 */
function RoutingSurfaceDiagram({ compact = false }: { compact?: boolean }) {
  return (
    <div className="relative">
      <svg
        viewBox="0 0 640 220"
        className={cn(
          "mx-auto h-auto w-full text-zinc-500",
          compact
            ? "max-h-[min(22vh,200px)] min-h-[140px]"
            : "max-h-[min(38vh,380px)] min-h-[210px]",
        )}
        aria-hidden
      >
        <defs>
...
export function LandingHeroOperationalViz({
  ...
  return (
    ...
    >
      <div
        className="landing-hero-scanlines pointer-events-none absolute inset-0 opacity-[0.1]"
        aria-hidden
      />
      <div
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-2 border-b border-white/[0.06] px-3 py-2 sm:px-4">
        ...
        <span className="inline-flex items-center gap-1.5 rounded border border-white/10 bg-white/[0.03] px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-zinc-300">
          <span
            className="size-1 rounded-sm bg-emerald-400/90 landing-motion-live-dot"
            aria-hidden
          />
          Live
        </span>
      ...
      <div
        className={cn(
          "relative px-2 py-3 sm:px-4 sm:py-4",
          !screenshotOnly && "border-b border-white/[0.06]",
        )}
      >
        <RoutingSurfaceDiagram compact={screenshotOnly} />
      </div>
        ...
        <div className="relative grid gap-3 p-3 sm:grid-cols-12 sm:gap-4 sm:p-4">
          <div className="rounded-md border border-white/[0.06] bg-black/40 p-3 sm:col-span-4">
            <p className="font-mono text-[9px] font-semibold uppercase tracking-[0.14em] text-zinc-500">
              Where the business still owns you
            </p>
            <ul className="mt-2 space-y-1.5">
              {FEED.map((line) => (
                <li
                  key={line}
                  className="font-mono text-[10px] leading-tight text-zinc-400"
                >
                  {line}
                </li>
              ))}
          ...
          <div className="grid grid-cols-3 gap-2 sm:col-span-8 sm:gap-3">
            <div className="rounded-md border border-rose-500/18 bg-rose-950/20 p-2.5 sm:p-3">
              <p className="font-mono text-[9px] font-semibold uppercase tracking-[0.12em] text-rose-300/85">
                Escape readiness
              </p>
              <p className="mt-1 text-xl font-semibold tabular-nums text-rose-100 sm:text-2xl">
                28%
              </p>
              <p className="mt-0.5 text-[10px] leading-snug text-zinc-500">
                Could run a week without you
              </p>
            </div>
            <div className="rounded-md border border-white/[0.06] bg-white/[0.02] p-2.5 sm:p-3">
              <p className="font-mono text-[9px] font-semibold uppercase tracking-[0.12em] text-zinc-500">
                Index
              </p>
              <p className="landing-motion-score mt-1 text-xl font-semibold tabular-nums text-white sm:text-2xl">
                68
              </p>
