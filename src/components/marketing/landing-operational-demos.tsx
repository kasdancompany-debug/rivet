      ...
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 overflow-hidden rounded-lg border border-zinc-200 ring-1 ring-zinc-950/[0.03] dark:border-zinc-800 dark:ring-white/[0.04] sm:grid sm:grid-cols-2">
          <div className="border-b border-zinc-200 bg-zinc-50 p-5 sm:border-b-0 sm:border-r sm:p-6 dark:border-zinc-800 dark:bg-zinc-900/35">
            <h3 className="font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-500">Before</h3>
            <ul className="mt-4 space-y-3">
              {before.map((line) => (
                <li key={line} className="flex gap-3 text-[13px] leading-snug text-zinc-600 dark:text-zinc-400">
                  <span className="mt-2 size-1 shrink-0 rounded-full bg-zinc-300 dark:bg-zinc-600" aria-hidden />
                  <span>{line}</span>
                </li>
              ))}
            </ul>
          ...
          <div className="bg-white p-5 sm:p-6 dark:bg-zinc-950">
            ...
            <ul className="mt-4 space-y-3">
              {after.map((line) => (
                <li key={line} className="flex gap-3 text-[13px] font-medium leading-snug text-zinc-900 dark:text-zinc-100">
                  <span className="mt-2 size-1 shrink-0 rounded-full bg-zinc-950 dark:bg-white" aria-hidden />
                  <span>{line}</span>
                </li>
              ))}
            </ul>
